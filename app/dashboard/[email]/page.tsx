    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: "rgba(26,43,60,0.5)" }}>
      <div className="card p-6 w-full max-w-sm">
        ...
        <div className="flex gap-3">
          ...
          <button className="btn-primary flex-1" onClick={onConfirm} disabled={deleting}
            style={{ background: "var(--error)" }}>{deleting ? "Deleting…" : "Delete"}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="card overflow-hidden">
      <div className="px-6 py-5" style={{ borderBottom: "1px solid var(--border)" }}>
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {room.balancePoint !== null
              ? <div className="text-right"><p className="text-xs" style={{ color: "var(--muted)" }}>Balance point</p><p className="font-display text-2xl font-semibold" style={{ color: "var(--sky)" }}>{room.balancePoint.toFixed(1)}°F</p></div>
        ...
        <div className="mt-4 flex gap-2">
          <Link href={`/edit/${room.id}`}
            style={{ display:"inline-block", padding:"7px 14px", borderRadius:8, border:"1.5px solid var(--border)", fontSize:"0.8rem", fontWeight:500, color:"var(--navy)", background:"var(--cream-dark)", textDecoration:"none" }}>
            ✏ Edit room
          </Link>
          <button type="button" onClick={onDelete}
            style={{ padding:"7px 14px", borderRadius:8, border:"1.5px solid var(--border)", fontSize:"0.8rem", fontWeight:500, color:"var(--error)", background:"var(--error-light)", cursor:"pointer" }}>
            🗑 Delete
          </button>
        </div>
      </div>
      ...
      <div className="px-6 py-5">
        {loading && <div className="flex items-center gap-2" style={{ color:"var(--muted)" }}><svg className="animate-spin" width="16" height="16" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeDasharray="32" strokeDashoffset="12"/></svg><span className="text-sm">Fetching recommendation…</span></div>}
        {error && !loading && <div className="text-sm px-4 py-3 rounded-xl" style={{ background:"var(--error-light)", color:"var(--error)" }}>{error} <button className="ml-3 underline text-xs" onClick={onRefresh}>Retry</button></div>}
        {!loading && !error && !rec && (
              ...
              <div className="space-y-2">
                ...
                {rec.openPeriods.map((p,i) => (
                  <div key={i} className="rounded-lg p-3" style={{ background:"var(--sky-light)", border:"1px solid var(--sky)" }}>
                    <p className="font-semibold text-sm" style={{ color:"var(--navy)" }}>{p.from} – {p.to}{p.multiDay && <span className="ml-2 text-xs px-2 py-0.5 rounded-full font-semibold" style={{background:"var(--sky-light)",color:"var(--sky)"}}>Multi-day</span>}</p>
  ...
  const loadRec = useCallback(async (roomId: string) => {
    ...
    try {
      ...
      setRoomStates(prev => prev.map(s => s.room.id === roomId ? { ...s, loading: false, rec: {
        ...
        openPeriods: postData.recommendation.openPeriods ? JSON.parse(postData.recommendation.openPeriods) : [],
        reasoning: postData.recommendation.reasoning,
        emailSent: postData.recommendation.emailSent,
        highF: postData.forecast?.highF, lowF: postData.forecast?.lowF, cityName: postData.forecast?.cityName,
      }} : s));
    } catch (err) {
